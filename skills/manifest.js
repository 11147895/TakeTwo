/**
 * TakeTwo — Skill 清单（轻量路由元数据）
 * 供系统路由/检索使用：判断何时用哪个方法、版本管理。
 * 完整步骤定义在 skills/<file> 中；manifest 只负责"被找到"。
 * 同步维护一份 manifest.json（同内容，供外部/工具直接读取）。
 * 注意：必须挂 window（不能用顶层 const），组装器通过 window.SKILL_MANIFEST 读取。
 */
window.SKILL_MANIFEST = [
  {
    id: "nvc",
    file: "nvc.js",
    name: "非暴力沟通",
